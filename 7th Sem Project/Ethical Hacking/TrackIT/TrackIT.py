...
def TrackLu():
    try:
        ...
        for site in social_media:
            ...
            try:
                response = session.get(url, timeout=10)  # Timeout to prevent long waits
                if response.status_code == 200:
                    results[site['name']] = url
                else:
                    results[site['name']] = f"{Ye}Username not found!{Ye}"
            except requests.exceptions.RequestException as e:
                # Log error for unreachable sites
...
def option():
    ...
    stderr.writelines(f"""
     ...
     /_/ /_/   \__,_/\___/_/|_| 
              ...
              {Wh}[ + ]  C O D E   BY ADESH  RAHI AABHASH  [ + ]
    """)
...
def run_banner():
    ...
    stderr.writelines(f"""{Wh}
         .-.
       .'   `.          {Wh}--------------------------------
       :z  z :         {Wh}| {Gr}GHOST - TRACKER - IP ADDRESS {Wh}|
       : --    `.        {Wh}|        Adesh Rahi and Aabhash      {Wh}|
      :         ``.     {Wh}--------------------------------
     :             `.
    :  :         .   `.
    :   :          ` . `.
